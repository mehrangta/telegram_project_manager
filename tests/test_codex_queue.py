...
class CodexQueueManagerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_renders_mixed_scoped_queue_and_escapes_dynamic_text(self):
        ...
        manager = CodexQueueManager(
            db=self.db, code_service=code, ask_service=asks, do_service=do
        )
        ...
        expected = [{"chat_id": 10, "thread_id": 30}]
    ...
    async def test_empty_scope_unexpected_arguments_and_permissions(self):
        ...
        manager = CodexQueueManager(
            db=self.db, code_service=empty, ask_service=empty, do_service=empty
        )
        ...
        unauthorized = await manager.handle(
            IncomingMessage(10, 99, "user", "/queue", thread_id=30)
        )
        ...
        self.assertEqual(unauthorized, "Unauthorized. Admin role required.")
