...
PRIMARY_ID_RE = re.compile(
    r"(?m)^(Code Job ID|Do Job ID|Goal ID|Draft ID|Plan ID):\s*([^\s]+)\s*$"
)
...
def keyboard_for_text(text: str) -> tuple[tuple[InlineButton, ...], ...]:
    ...
    noun = {"Code Job ID": "Job ID", "Do Job ID": "Job ID", "Goal ID": "Goal ID", "Draft ID": "Draft ID", "Plan ID": "Plan ID"}[
        primary.group(1)
    ]
...
def _render_line(line: str) -> str:
    ...
    elif label in {"Code Job ID", "Do Job ID", "Goal ID", "Draft ID", "Plan ID", "Commit", "Context commit", "Merge commit"}:
