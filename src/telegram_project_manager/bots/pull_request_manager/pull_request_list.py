...
PULL_REQUEST_LIST_REFRESH_SECONDS = 60.0


class PullRequestListError(RuntimeError):
    pass
...
class PullRequestListService:
    def __init__(
        ...
        *,
        db: Database,
        bot: TelegramBotApi,
        reader: GhPullRequestReader,
        interval_seconds: float = PULL_REQUEST_LIST_REFRESH_SECONDS,
    ) -> None:
        self.db = db
        self.bot = bot
        self.reader = reader
        ...
        async with self._locks[(chat_id, thread_id)]:
            try:
                pull_requests = await asyncio.to_thread(
                    self.reader.list_open_pull_requests, repo
                )
                outgoing = self.render(repo, pull_requests)
                ...
                raise PullRequestListError(_safe_error(exc)) from exc

            try:
                self.db.upsert_pull_request_list_message(
                    chat_id,
                    thread_id,
                    message_id,
                    repo,
                ...
                try:
                    ...
                    logging.exception(
                        "Failed to remove untracked pull request list: chat_id=%s message_id=%s",
                        chat_id,
                        message_id,
                    )
                raise PullRequestListError(_safe_error(exc)) from exc
    ...
    async def recover(self) -> None:
        if self._task is not None and not self._task.done():
            return
        self._task = asyncio.create_task(self._run(), name="pull-request-list-refresh")
    ...
    async def refresh(self) -> None:
        targets = self.db.list_pull_request_list_messages()
        grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for target in targets:
            repo = str(target["repo"])
            if not self.db.is_repo_allowed(repo):
                self.db.delete_pull_request_list_message(
    ...
    async def _run(self) -> None:
        while True:
            try:
                ...
                logging.exception("Unexpected pull request list refresh failure")
    ...
    async def _refresh_target(
        ...
        repo = str(target["repo"])
        async with self._locks[(chat_id, thread_id)]:
            current = self.db.get_pull_request_list_message(chat_id, thread_id)
    ...
    def _audit_refresh_failure(
        ...
        self.db.audit("prs.refresh", "failed", details)

    @staticmethod
    def render(repo: str, pull_requests: list[PullRequestSummary]) -> OutgoingMessage:
        escaped_repo = html.escape(repo)
        if not pull_requests:
            return OutgoingMessage(text=f"No open pull requests for {escaped_repo}.")

        lines = [f"Open pull requests for {escaped_repo}:"]
        for pull_request in pull_requests:
            lines.append(
                f'- <a href="{html.escape(pull_request.url, quote=True)}">'
                f"#{pull_request.number}</a> — {html.escape(pull_request.title)}"
            )
